...
//				delayedClose : 500 ,
			onShow : function(ct, ci) { // Outer/inner jQuery node
				...
				if ( self.hasNoLabelInMainLanguage(i) ) {
					...
					h += "</i>" ;
					if ( self.allowLabelOauthEdit ) {
						let url = '/index.php?action=authorize&path=?q='+self.q ;
						h += "<br/><a href='#' onclick='reasonator.addLabelOauth(\""+q+"\",\""+pl+"\",\""+escattr(i.getLabel())+"\");return false'><b>" ;
						h += tt.t('add_a_label') + "</b></a>" ;
						h += " (" + tt.t('via_widar').replace(/\$1/,"<a href='"+url+"'>log in</a>") + ")" ;
					}
					h += "</div>" ;
				}
	...
	addLabelOauth : function ( q , lang , basis , callback ) {
		...
		var oauth_wait_dialog = "<div id='oauth_wait_dialog' style='width: 200px; position: absolute; top: 200px; background-color: #99C7FF; color: white; text-align: center; left: 50%; margin-left: -100px; padding: 5px;'>Editing...</div>" ;
